// 2022_02_14_winApI2D.cpp : 애플리케이션에 대한 진입점을 정의합니다.
//

#include "pch.h"
#include "2022_02_14_winApI2D.h"
                     ...
                     _In_ int       nCmdShow)
{
    ...
    LoadStringW(hInstance, IDC_MY20220214WINAPI2D, szWindowClass, MAX_LOADSTRING);
    ...
    // GetMessage : 메세지 큐에 메세지가 없다면 대기, 메세지가 들어온다면 true 반환
    // PeekMessage : 메세지 큐에 메세지가 없다면  false, 메세지가 있다면 true 반환

    // 게임 루프
    // GetMessage의 대기 상태 유지에서
    // 현재 PeekMessage의 메세지가 없는 상황에서 게임 상황 처리
    MSG msg;
    while (true)
    {
        if (PeekMessage(&msg, nullptr, 0, 0, PM_REMOVE))
        {
            ...
            if (!TranslateAccelerator(msg.hwnd, hAccelTable, &msg)) // 단추키에 대한 처리
            {
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
    ...
    wcex.hIcon          = LoadIcon(hInstance, MAKEINTRESOURCE(IDI_MY20220214WINAPI2D)); // 프로그램 아이콘
    ...
    wcex.hbrBackground  = (HBRUSH)(COLOR_WINDOW+1);                                     // 윈도우 작업영역애 칠한 배경 브러시
    wcex.lpszMenuName   = nullptr;                                                      // 윈도우에서 사용할 메뉴 지정, nullptr 로 없앰
